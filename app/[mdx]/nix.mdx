# NixOS has a couple gotchas

## It might be nicer than Ubuntu, even for normies

Things that were randomly broken on my Ubuntu install that now "just work":

* `bash` PS1 colors on `sh`
* external monitor no longer force reconnects whenever I open GNOME settings or
  some apps
* xterm no longer randomly freezes until input once every sometimes

There's the one caveat though:

## npm i --global is strictly forbidden

https://nixos.wiki/wiki/Node.js#Using_npm_install_-g_fails

Where `npm i -g` is shorthand for "a lot of types of installs".
