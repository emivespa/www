...
For longer scripts it's very practical to emulate the structure of the `git` CLI.
A bunch of independent commands where you can call the "porcelain" ones as well
as "plumbing" ones.
...
# [flag parsing]
# [commands that MUST run on every run]

"${1:-main}" # Calls the specified function or main if there isn't one.
```

You can test every function independently by running `./script foo`,
`./script bar`.
